...
s32 check_kick_or_dive_in_air(struct MarioState *m) {
    if (m->input & INPUT_B_PRESSED && SM64AP_CanDoAction(ACT_JUMP_KICK)) {
        return set_mario_action(
            m, m->forwardVel > 28.0f && SM64AP_CanDoAction(ACT_DIVE) ? ACT_DIVE : ACT_JUMP_KICK,
            0);
    }
    return FALSE;
}
...
s32 act_jump(struct MarioState *m) {
    ...
    if (m->input & INPUT_Z_PRESSED && SM64AP_CanDoAction(ACT_GROUND_POUND)) {
...
s32 act_triple_jump(struct MarioState *m) {
    ...
    if (m->input & INPUT_B_PRESSED && SM64AP_CanDoAction(ACT_DIVE)) {
...
s32 check_wall_kick(struct MarioState *m) {
    if ((m->input & INPUT_A_PRESSED) && SM64AP_CanDoAction(ACT_WALL_KICK_AIR) && m->wallKickTimer != 0 && m->prevAction == ACT_AIR_HIT_WALL) {
        m->faceAngle[1] += 0x8000;
...
s32 act_air_hit_wall(struct MarioState *m) {
    ...
    if (++(m->actionTimer) <= 2) {
        if (m->input & INPUT_A_PRESSED && SM64AP_CanDoAction(ACT_WALL_KICK_AIR)) {
            m->vel[1] = 52.0f;
            m->faceAngle[1] += 0x8000;
            return set_mario_action(m, ACT_WALL_KICK_AIR, 0);
        }
